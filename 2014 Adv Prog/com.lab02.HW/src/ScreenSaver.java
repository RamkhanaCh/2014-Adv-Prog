...
// My code seems to work just fine for me but when I let my friend run my program, it doesn't work properly. 
//The problem is she tries to run it several times and find out that sometimes the dialog is not appear on the applet.
//Just in case if you run it and the same problem occurs. This is youtube link for the video that capture my screen when the applet is run.
//http://www.youtube.com/watch?v=MUzrtsZx3UE&feature=youtu.be
...
public class ScreenSaver extends JApplet implements Runnable {
	...
	private int[] randomShape() {
		for (int i = 0; i < MAXOBJECTS; i++) {
			type[i] = ranDomShapeID.nextInt(2);
		}
		return type;
	}
	...
	private boolean isInWindow(int shapeNumber) {
		Rectangle2D tempPic = new Rectangle2D.Double(
				properties[shapeNumber][1], properties[shapeNumber][2],
				properties[shapeNumber][3], properties[shapeNumber][3]);
