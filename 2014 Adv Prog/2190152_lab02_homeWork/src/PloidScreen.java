...
public class PloidScreen extends JApplet implements Runnable {
	...
	int[][] shapeProperties = new int[MAXOBJECTS][4]; // store the properties of
														// each shape {{circle
														// or rectangle, x , y,
														// size},....}
	...
	int counter = 0; // create a counter for the program to know that it is
						// first time that paint or not (if yes show dialogue
						// box)
				...
				if (shapeProperties[i][0] == 0) {
					...
					if (upOrDowns[i] == -1) {
						...
						shapes[i] = new Rectangle2D.Double(
								shapeProperties[i][1],
								shapeProperties[i][2] -= yPos,
								shapeProperties[i][3], shapeProperties[i][3]);
					...
					if (upOrDowns[i] == -1) {
						...
						shapes[i] = new Ellipse2D.Double(shapeProperties[i][1],
								shapeProperties[i][2] -= yPos,
								shapeProperties[i][3], shapeProperties[i][3]);
	...
	private boolean isInMargin(int i) {
		...
		Rectangle2D temp = new Rectangle2D.Double(shapeProperties[i][1],
				shapeProperties[i][2], shapeProperties[i][3],
				shapeProperties[i][3]);
	...
	private boolean testIntersection(int i) {
		...
		for (int j = 0; j < i; j++) {
			...
			temp = temp
					|| shapes[i].getBounds2D().intersects(
							shapes[j].getBounds2D());
	...
	private Shape getRandRect(int i) {
		...
		int squareProperty[] = getShapeProperty(); // method to return the array
													// of {x coordinate, y
	...
	private Shape getRandCircle(int i) {
		...
		int circleProperty[] = getShapeProperty(); // method to return the array
													// of {x coordinate, y
													// coordinate, side length}
	...
	private int[] getShapeProperty() {
		...
		while ((XRIGHTMOST - tempX - 5) < (sideLength)
				|| (YBOTTOMMOST - tempY - 5) < (sideLength)) {
